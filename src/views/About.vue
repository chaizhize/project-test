                    ...
                    <ul class="remarks">
                        <li @click.stop="handleResetClick(item, index)" v-for="(item, index) in remarkList" :key="index">
                            <div v-if="item.isShow">
                                <div style="margin-bottom: 5px;" v-html="item.selectionText"></div>
                                <div @click.stop="handleDel(item, index)">删除</div>
                                <div v-html="item.value" :class="item.color" v-if="item.type != 3"></div>
                                <div></div>
                            </div>
                        </li>
                    </ul>
        ...
        correct({ type, color, ...item }, index, state) {
            this.$prompt('', state === 'add' ? '添加批注' : '编辑批注', {
                ...
                .then(({ value }) => {
                    ...
                    if (state === 'add') {
                        var { id, text, selectionText } = this.TextCorrect.markContent(color);
                        this.abc = text;
